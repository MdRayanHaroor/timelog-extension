  ...
  taskList.innerHTML = tasks.map((task) => {
    ...
    const actionButtons = isToday
      ? `<div class="task-actions">
           ...
           <button class="delete-btn" data-logid="${task.logId}" title="Delete">🗑️</button>  
         </div>`
      : "";
...
//         <button class="save-edit-btn" data-logid="${taskItem.dataset.logid}" title="Save">💾</button>  // CHANGED: Use data-logid
...
//   try {
//       const tasks = await getTasksForDate(date);
//       const otherTasks = tasks.filter(t => t.logId !== logIdToEdit);  // CHANGED: Use logId
//       const loggedMinutes = otherTasks.reduce((total, task) => total + (task.hours * 60) + task.minutes, 0);
...
//       const taskToUpdate = tasks.find(t => t.logId === logIdToEdit);  // CHANGED: Use logId
//       taskToUpdate.hours = newHours;
//       taskToUpdate.minutes = newMinutes;
//       taskToUpdate.description = newDescription;
...
async function handleSaveEdit(saveButton) {
  ...
  try {
      ...
      // Add validation to ensure task exists
      if (!taskToUpdate) {
...
function handleTaskListActions(e) {
  ...
  if (target.classList.contains("delete-btn")) handleDeleteTask(logId);
  else if (target.classList.contains("edit-btn")) handleEditTask(target);
  else if (target.classList.contains("save-edit-btn")) handleSaveEdit(target);
  else if (target.classList.contains("cancel-edit-btn")) handleCancelEdit();
}
...
async function handleDeleteTask(logId) {
  const date = document.getElementById("filterDate").value;
  if (!date || !confirm("Are you sure you want to delete this time log?")) return;
  
  console.log("[handleDeleteTask] Deleting logId:", logId, "for date:", date);
  
  try {
    // For delete, we need to send the logId to a delete endpoint
    const response = await fetch("https://chrome-ext-timelogger-dbc8ctcbe8f7gpc4.centralindia-01.azurewebsites.net/api/deleteLog", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ Log_Id: logId })
    });
    
    console.log("[handleDeleteTask] Response status:", response.status);
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error("[handleDeleteTask] Error response:", errorText);
...
async function savePATSettings() {
  ...
  try {
    ...
    catch (err) { showMessage("loadOrganizations failed:", err); }

    try { await autoPopulateOrganization(); } 
    catch (err) { showMessage("autoPopulateOrganization failed:", err); }
