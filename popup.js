/* global chrome, graphAuth, graphStorage, getADOSettings, saveADOSettings, clearADOSettings, getTasksForDate, saveTasksForDate, initializeGraphAuth, fetchOrganizations, fetchProjects, fetchWorkItems, showMessage, formatTime, displayDailyTotal, togglePATVisibility, showLoading */
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("authenticateGraph").addEventListener("click", handleGraphAuthentication)
...
async function addTask() {
  ...
  try {
    ...
    if (loggedMinutes + newMinutes > totalMinutesLimit) {
      ...
      showMessage(
        remainingMinutes <= 0
          ? "You have already logged 8 hours for this day."
          : `Exceeds 8-hour limit. You can only log ${remainingHours}h ${remainingMins}m more.`,
        "error",
      )
      return
    }
...
async function handleSaveEdit(saveButton) {
  ...
  const newHours = Number.parseInt(taskItem.querySelector(".edit-hours").value, 10) || 0
  const newMinutes = Number.parseInt(taskItem.querySelector(".edit-minutes").value, 10) || 0
...
function applyFiltersToTasks(tasks, date) {
  ...
  if (selectedWorkItem) {
    const workItemId = Number.parseInt(selectedWorkItem.split(":")[1], 10)
...
async function handleRetryClick() {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({ type: "WORK_ITEM_DETECTED", workItem: workItemData }, (response) =>
...
function updateSettingsSummary(expiryDate) {
  document.getElementById("settingsSummary").textContent =
    `Settings - PAT expires on ${expiryDate.toISOString().split("T")[0]}`
}
...
async function clearPATSettings() {
  ...
  document.getElementById("graphAuthStatus").textContent = ""
  document.getElementById("authenticateGraph").textContent = "Authenticate with Microsoft"
...
async function handleGraphAuthentication() {
  const authBtn = document.getElementById("authenticateGraph")
  const statusDiv = document.getElementById("graphAuthStatus")

  try {
    authBtn.disabled = true
    authBtn.textContent = "Authenticating..."
    statusDiv.textContent = "Authenticating with Microsoft..."

    await graphAuth.authenticate()

    statusDiv.textContent = "✅ Authenticated successfully"
    statusDiv.style.color = "green"
    authBtn.textContent = "Re-authenticate"

    showMessage("Microsoft Graph authentication successful!", "success")
  } catch (error) {
    ...
    showMessage("Microsoft Graph authentication failed. Please try again.", "error")
  } finally {
...
const initializeGraphAuth = async () => {}
const showMessage = (message, type) => {}
const getTasksForDate = async (date) => []
const saveTasksForDate = async (date, tasks) => {}
const formatTime = (hours, minutes) => `${hours}h ${minutes}m`
const displayDailyTotal = (tasks) => {}
const togglePATVisibility = () => {}
const showLoading = (id, isLoading) => {}
const fetchOrganizations = async (pat) => {}
const fetchProjects = async (pat, org) => {}
const fetchWorkItems = async (pat, org, projectId, query) => {}
const graphAuth = {
  authenticate: async () => {},
}
const getADOSettings = async () => {}
const saveADOSettings = async (settings) => {}
const clearADOSettings = async () => {}
