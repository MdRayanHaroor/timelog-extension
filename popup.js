...
async function handleRetryClick() {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      window.chrome.runtime.sendMessage({ type: "WORK_ITEM_DETECTED", workItem: workItemData }, (response) =>
        window.chrome.runtime.lastError ? reject(window.chrome.runtime.lastError) : resolve(response),
      )
    })
...
async function savePATSettings() {
  console.log("[v0] Save PAT button clicked")

  const pat = document.getElementById("pat").value.trim()
  const expiryDate = document.getElementById("expiryDate").value
  const organization = document.getElementById("organization").value.trim()

  console.log("[v0] PAT value:", pat ? "***hidden***" : "empty")
  ...
  console.log("[v0] Attempting to save settings...")
...
async function checkAndPreSelectWorkItem() {
  try {
    const response = await new Promise((resolve, reject) => {
      window.chrome.runtime.sendMessage({ type: "GET_CURRENT_WORK_ITEM" }, (response) => {
...
function displayDailyTotal(tasks) {
  const totalMinutes = tasks.reduce((total, task) => total + task.hours * 60 + task.minutes, 0)
  const totalHours = Math.floor(totalMinutes / 60)
  const remainingMinutes = totalMinutes % 60
  const dailyTotalElement = document.getElementById("dailyTotal")
  dailyTotalElement.textContent = `Total logged: ${totalHours}h ${remainingMinutes}m`
}
...
async function getTasksForDate(date) {
  // Implementation for fetching tasks for a specific date
}

async function saveTasksForDate(date, tasks) {
  // Implementation for saving tasks for a specific date
}

async function getADOSettings() {
  // Implementation for fetching Azure DevOps settings
}

async function saveADOSettings(settings) {
  // Implementation for saving Azure DevOps settings
}

async function clearADOSettings() {
  // Implementation for clearing Azure DevOps settings
}

async function fetchOrganizations(pat) {
  // Implementation for fetching organizations using PAT
}

async function fetchProjects(pat, org) {
  // Implementation for fetching projects using PAT and organization
}

async function fetchWorkItems(pat, org, projectId, query) {
  // Implementation for fetching work items using PAT, organization, project ID, and query
}
